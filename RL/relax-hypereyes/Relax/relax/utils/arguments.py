    ...
    def add_slime_arguments(parser):
        ...
        def add_on_policy_distillation_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Forward KL coefficient for Tri-branch Outcome-Conditioned OPD (branch B: correct + tool-used). "
                    "When set, branch-B samples use Forward KL (Teacher ∥ Student) with this coefficient, "
                    "distilling efficient trajectories into the student. "
                    "Defaults to --opd-kl-coef when not specified."
                ),
            )
            parser.add_argument(
                "--opd-only-reward",
