...
def convert_samples_to_train_data(args: Any, samples: list[Sample] | list[list[Sample]]):
    ...
    # Tri-branch OPD: propagate opd_branch from reward dict to train_data.
    # reward_func returns {"score": float, "opd_branch": "reverse_kl"/"forward_kl"/"skip"}.
    # This field is later consumed by apply_opd_kl_to_advantages to choose KL direction.
    # NOTE: opd_branch is a list[str], excluded from dict_to_tensordict (non-numeric).
    #       It is re-attached to rollout_data in post_process_rollout_data / stream_dataloader.
    if any(
        isinstance(sample.reward, dict) and "opd_branch" in sample.reward
        for sample in samples
    ):
        train_data["opd_branch"] = [
            (sample.reward.get("opd_branch", "skip") if isinstance(sample.reward, dict) else "skip")
            for sample in samples
        ]
    ...
    # Separate non-numeric fields that cannot be stored in TensorDict.
    # These are re-attached as plain Python lists after tensordict creation.
    _non_tensor_keys = ["opd_branch"]
    non_tensor_data = {k: train_data.pop(k) for k in _non_tensor_keys if k in train_data}

    rollout_batch = dict_to_tensordict(train_data, len(total_lengths))

    # Re-attach non-tensor fields to the rollout_batch dict-like object.
...
def post_process_rewards(args: Any, samples: list[Sample] | list[list[Sample]]):
    ...
    Anomaly handling: samples with raw_reward <= -99 are treated as INVALID.
    They will:
      1) NOT participate in their group's mean/std calculation
         (avoid contaminating siblings in the same group)
      2) Their normalized reward is set to 0.0 (zero advantage downstream)
      3) raw_rewards stays unchanged so downstream pass@k / accuracy stats
         can still identify and exclude them separately.

    Returns:
        Tuple[List[float], List[float]]: (raw_rewards, normalized_rewards)
    """
    if args.custom_reward_post_process_path is not None:
        custom_reward_post_process_func = load_function(args.custom_reward_post_process_path)
        return custom_reward_post_process_func(args, samples)
    ...
    # ============================================================
    # Group-wise normalization with anomaly-aware masking
    # ============================================================
    rewards = torch.tensor(raw_rewards, dtype=torch.float)
    ...
    # ★ Step 1: Identify anomaly samples (robust to float precision)
    is_anomaly = rewards <= -99.0                 # [num_groups, group_size]
    is_valid = ~is_anomaly
    valid_count = is_valid.sum(dim=-1, keepdim=True).float()  # [num_groups, 1]

    # ★ Step 2: Compute group mean using ONLY valid samples
    # Trick: zero-out anomaly positions, then divide by valid_count
    rewards_for_stats = torch.where(is_valid, rewards, torch.zeros_like(rewards))
    safe_count = torch.clamp(valid_count, min=1.0)
    mean = rewards_for_stats.sum(dim=-1, keepdim=True) / safe_count  # [num_groups, 1]

    # Center: (reward - group_mean), but anomaly positions will be overridden later
    rewards = rewards - mean

    # ★ Step 3: Compute group std using ONLY valid samples (if needed)
    if args.advantage_estimator in ["grpo", "gspo", "sapo"] and args.grpo_std_normalization:
        # Squared deviations, but only count valid positions
        sq_dev = (rewards * rewards) * is_valid.float()
        # Unbiased estimator (n-1) to match torch.std default behavior;
        # fall back to n=1 to avoid division by zero in degenerate groups.
        std_denom = torch.clamp(valid_count - 1, min=1.0)
        var = sq_dev.sum(dim=-1, keepdim=True) / std_denom
        std = torch.sqrt(var)
        rewards = rewards / (std + 1e-6)

    # ★ Step 4: Force anomaly samples to 0.0 (zero advantage)
    rewards = torch.where(is_valid, rewards, torch.zeros_like(rewards))

    # ★ Step 5: Degenerate groups handling
    # If a group has <= 1 valid sample, std is undefined / advantage meaningless
    # → zero out the whole group (those samples should also be loss-masked downstream)
    degenerate = (valid_count <= 1).expand_as(rewards)
    if degenerate.any():
        rewards = torch.where(degenerate, torch.zeros_like(rewards), rewards)

    # ★ Step 6: Logging (sampled to avoid spam)
    n_anomaly = int(is_anomaly.sum().item())
    n_degenerate_groups = int((valid_count.squeeze(-1) <= 1).sum().item())
    if n_anomaly > 0 or n_degenerate_groups > 0:
        logger.warning(   # ← 直接用模块级的 logger
            f"[post_process_rewards] anomaly_samples={n_anomaly}/{rewards.numel()}, "
            f"degenerate_groups(valid<=1)={n_degenerate_groups}/{rewards.shape[0]}, "
            f"group_size={rewards.shape[1]}"
        )
