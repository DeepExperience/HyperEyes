...
class Advantages(Base):
    ...
    def compute_advantages_and_returns(self, rollout_data: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if raw_rewards is not None:
            ...
            if invalid_indices:
                self._logger.info(  
                    f"[compute_advantages_and_returns] masked {len(invalid_indices)} "
                    f"anomaly samples (raw_reward<=-99) at indices {invalid_indices}"
                )
        # ============================================================
        ...
        if getattr(self.config, "use_opd", False):
            apply_opd_kl_to_advantages(
                ...
                invalid_indices=invalid_indices,  # pass through for Tri-branch zero-out
            )

        result = {
            "advantages": torch.nested.nested_tensor(advantages),
            "returns": torch.nested.nested_tensor(returns),
            "loss_masks": torch.nested.nested_tensor(loss_masks),  # ★ 必须把更新后的 loss_masks 写回！
        }
