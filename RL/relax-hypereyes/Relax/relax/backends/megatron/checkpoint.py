...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, checkpointing_context, skip_load_to_model_and_opt):
    ...
    if exist and _is_megatron_checkpoint(load_path):
        ...
        # )
        with _patch_no_save_optim_load(args, optimizer):
            return _load_checkpoint_megatron(
                ddp_model=ddp_model,
                optimizer=optimizer,
                opt_param_scheduler=opt_param_scheduler,
...
@contextmanager
def _patch_no_save_optim_load(args, optimizer):
    """Workaround for old Megatron versions that do not properly respect
    ``--no-save-optim`` when loading a distributed checkpoint.

    Newer Megatron (>= certain commit) guards the optimizer-load block with::

        if (... and not getattr(state_dict['args'], 'no_save_optim', False)):

    Older versions on Ray worker nodes (/root/Megatron-LM) skip that guard and
    directly call ``optimizer.load_state_dict(state_dict['optimizer'])``, which
    raises ``KeyError: 'optimizer'`` because the checkpoint was saved without
    optimizer state.

    Fix strategy (no-save-optim path only):
      1. Force ``args.no_load_optim = True`` so that any version of Megatron
         that *does* check this flag will skip optimizer loading.
      2. Additionally replace ``optimizer.load_state_dict`` with a no-op at the
         instance level so that even versions that ignore the flag are safe.
      3. After ``_load_checkpoint_megatron`` returns, restore everything and
         call ``optimizer.reload_model_params()`` (the same call Megatron makes
         in its own ``no_load_optim`` else-branch) so fp16/bf16 master weights
         are refreshed from the freshly-loaded model weights.
    """
    no_save_optim = getattr(args, "no_save_optim", False)
    if not no_save_optim or optimizer is None:
        # Nothing to patch — yield and return.
        yield
        return

    logger.info(
        "no_save_optim=True detected on a Megatron checkpoint resume. "
        "Temporarily patching optimizer to skip optimizer state loading "
        "(workaround for old Megatron versions that ignore --no-save-optim on load)."
    )

    # 1. Force the flag in the global args object.
    original_no_load_optim = args.no_load_optim
    args.no_load_optim = True

    # 2. Patch load_state_dict at the instance level (overrides class method).
    original_load_state_dict = optimizer.__dict__.get("load_state_dict", None)
    optimizer.load_state_dict = lambda *a, **kw: None  # type: ignore[method-assign]
    ...
    try:
        ...
        if original_load_state_dict is None:
            optimizer.__dict__.pop("load_state_dict", None)
        else:
            optimizer.load_state_dict = original_load_state_dict

        # Refresh fp16/bf16 master weights from the model weights that were
        # just loaded — equivalent to what Megatron does in its no_load_optim
        # else-branch.
        if (args.fp16 or args.bf16) and not getattr(args, "load_main_params_from_ckpt", False):
