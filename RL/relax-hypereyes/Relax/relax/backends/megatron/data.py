...
def log_rollout_data(
    ...
    Anomaly handling:
      Samples with raw_reward <= -99 are treated as INVALID and excluded from
      all averaging statistics to avoid contamination of training metrics.
      Their loss_masks have been zeroed upstream, so token-level metrics are
      naturally masked out; we additionally correct the denominator to reflect
      the true valid sample count.
    """
    if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
        cp_size = mpu.get_context_parallel_world_size()
        log_dict = {}
        ...
        # OPD dynamic metric (README Eq. 6): overlap ratio on top-k token sets.
        # Keep the whole logic closed within this function as requested.
        student_topk_ids = rollout_data.get("topk_token_ids")
        teacher_topk_ids = rollout_data.get("teacher_topk_token_ids")
        if (
            isinstance(student_topk_ids, list)
            and isinstance(teacher_topk_ids, list)
            and len(student_topk_ids) > 0
            and len(student_topk_ids) == len(teacher_topk_ids)
            and all(isinstance(x, torch.Tensor) for x in student_topk_ids)
            and all(isinstance(x, torch.Tensor) for x in teacher_topk_ids)
            and isinstance(loss_masks, list)
            and len(loss_masks) > 0
            and all(isinstance(x, torch.Tensor) for x in loss_masks)
            and isinstance(total_lengths, list)
            and all(isinstance(x, int) for x in total_lengths)
            and isinstance(response_lengths, list)
            and all(isinstance(x, int) for x in response_lengths)
        ):
        ...
        # ============================================================
        # ★ 预先识别异常样本（raw_reward <= -99）
        #   这些样本会被排除在统计之外，避免污染训练监控指标
        # ============================================================
        raw_rewards_for_mask = rollout_data.get("raw_reward")
        invalid_indices: set[int] = set()
        if raw_rewards_for_mask is not None:
            for i, r in enumerate(raw_rewards_for_mask):
                r_val = float(r) if torch.is_tensor(r) else float(r)
                if r_val <= -99.0:
                    invalid_indices.add(i)

        n_total_samples = len(loss_masks)
        n_valid_samples = n_total_samples - len(invalid_indices)

        # Global valid sample count for distributed logging
        if mpu.get_data_parallel_world_size(with_context_parallel=True) > 1:
            n_valid_samples_tensor = torch.tensor(
                [n_valid_samples], dtype=torch.long, device=torch.cuda.current_device()
            )
            dist.all_reduce(
                n_valid_samples_tensor, op=dist.ReduceOp.SUM, group=mpu.get_data_parallel_group(with_context_parallel=True)
            )
            n_valid_samples_global = n_valid_samples_tensor.item()
            n_total_samples_tensor = torch.tensor(
                [n_total_samples], dtype=torch.long, device=torch.cuda.current_device()
            )
            dist.all_reduce(
                n_total_samples_tensor, op=dist.ReduceOp.SUM, group=mpu.get_data_parallel_group(with_context_parallel=True)
            )
            n_total_samples_global = n_total_samples_tensor.item()
        else:
            n_valid_samples_global = n_valid_samples
            n_total_samples_global = n_total_samples

        # 暴露异常样本的统计到日志，方便监控
        if n_total_samples_global > 0:
            log_dict["anomaly/count"] = float(n_total_samples_global - n_valid_samples_global)
            log_dict["anomaly/ratio"] = (n_total_samples_global - n_valid_samples_global) / n_total_samples_global
        # ============================================================
            ...
            if isinstance(val, (list, tuple)):
                if isinstance(val[0], torch.Tensor):
                    ...
                    if key in [
                        ...
                        # ★ 分母修正：异常样本的 loss_mask 已被清零，
                        #   sum_of_sample_mean 对它们贡献 0；若分母仍用 n_total，
                        #   平均值会被这些"贡献 0"的样本拉低。
                        #   改用 n_valid 才是真正的"有效样本平均值"。
                        denom = max(n_valid_samples_global, 1)
                        val = cp_size * sum_of_sample_mean(val) / denom
                    else:
                        val = torch.cat(val).clone().detach()
                        val = val.mean() * cp_size
                else:
                    # ★ 标量列表路径（如 raw_reward, rewards, truncated, round_number 等）
                    #   过滤掉异常索引后再求平均，避免 -100 污染
                    if invalid_indices:
                        filtered = [v for i, v in enumerate(val) if i not in invalid_indices]
                        if filtered:
                            val = sum(filtered) / len(filtered)
                        else:
                            # 整个 batch 都是异常样本（极端情况）
                            val = 0.0
                    else:
                        val = sum(val) / len(val)
            elif isinstance(val, torch.Tensor):
    ...
    if args.log_correct_samples:
        if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
            ...
            raw_rewards = rollout_data["raw_reward"]
            # ★ Additional metrics for correct cases are calculated separately below.
            #   注意：这里 `if raw_reward == 1` 已经天然排除了 -100 异常样本，
            #   因为异常样本的 raw_reward == -100，永远不会等于 1。
            #   所以这里不需要额外修改。
            correct_response_lengths = []
            correct_total_lengths = []
            correct_loss_masks = []
            correct_entropy = []
            for i, raw_reward in enumerate(raw_rewards):
                # ★ 安全的浮点比较（避免 nested tensor / float 类型差异）
                r_val = float(raw_reward) if torch.is_tensor(raw_reward) else float(raw_reward)
                if r_val == 1:
                    correct_response_lengths.append(response_lengths[i])
                    correct_total_lengths.append(total_lengths[i])
                    correct_loss_masks.append(loss_masks[i])
...
def log_passrate(rollout_id, args, rollout_data, ignore_num_groups=False):
    if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
        log_dict = {}
        for key, val in rollout_data.items():
            if key != "raw_reward":
            ...
            # ★ 把 -100 异常样本替换为 0（视作"未通过"）
            cleaned_val = []
            n_replaced = 0
            for r in val:
                r_f = float(r) if torch.is_tensor(r) else float(r)
                if r_f <= -99.0:
                    cleaned_val.append(0.0)
                    n_replaced += 1
                else:
                    cleaned_val.append(r_f)

            if n_replaced > 0:
                logger.info(
                    f"[log_passrate] replaced {n_replaced}/{len(val)} anomaly rewards "
                    f"with 0.0 for pass@k calculation"
                )
                log_dict["passrate/anomaly_ratio"] = n_replaced / len(val)

            log_dict |= compute_pass_rate(
                flat_rewards=cleaned_val,
                group_size=args.n_samples_per_prompt,
                num_groups=None if ignore_num_groups else args.rollout_batch_size,
            )
