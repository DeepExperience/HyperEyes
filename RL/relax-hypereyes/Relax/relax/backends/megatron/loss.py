...
def apply_opd_kl_to_advantages(
    ...
    1. **Standard mode** (no "opd_branch" in rollout_data):
       Computes Reverse KL (student_logp - teacher_logp) uniformly for all
       samples and subtracts ``opd_kl_coef * reverse_kl`` from advantages.

    2. **Tri-branch Outcome-Conditioned mode** (when "opd_branch" present):
       Applies different KL direction per sample based on the "opd_branch" field
       populated by the reward function (e.g., HyperEyes_reward_sft_dynamic):

         - "reverse_kl" (acc ≤ 0, wrong answer / parse error):
             penalty = +opd_kl_coef * (student_logp - teacher_logp)
             Effect: Teacher corrects the student's bad trajectory token-by-token,
             quickly suppressing invalid exploration space.

         - "forward_kl" (acc > 0 and tc > 0, correct + used tools):
             penalty = +opd_kl_coef * (teacher_logp - student_logp)
             Effect: Teacher distills its efficient trajectory distribution into the
             student, guiding it toward high-quality solutions.

         - "skip" (acc > 0 and tc == 0, lucky guess without tools):
             penalty = 0  (no OPD signal)
             Effect: Avoid reinforcing guessing behavior.

    Args:
        args: Configuration containing `use_opd`, `opd_kl_coef`, and optionally
            `opd_forward_kl_coef` (defaults to `opd_kl_coef` when absent).
        rollout_data: Dict containing "teacher_log_probs" and optionally
            "opd_branch" (list[str], one per sample).
        advantages: List of advantage tensors to modify in-place.
        student_log_probs: List of student log-probability tensors.
        invalid_indices: List of indices for anomaly samples (raw_reward <= -99) to skip.
    ...
    invalid_indices_set = set(invalid_indices) if invalid_indices is not None else set()

    # Tri-branch mode: per-sample OPD branch from reward_func output.
    # Falls back to uniform reverse-KL when field is absent (backward compat).
    opd_branches: list[str] | None = rollout_data.get("opd_branch", None)

    # Forward KL coefficient: use dedicated arg if available, else share opd_kl_coef.
    reverse_kl_coef = float(args.opd_kl_coef)
    _forward_kl_coef_raw = getattr(args, "opd_forward_kl_coef", None)
    forward_kl_coef = float(_forward_kl_coef_raw if _forward_kl_coef_raw is not None else args.opd_kl_coef)

    reverse_kls = []
    for i, adv in enumerate(advantages):
        if i in invalid_indices_set:
            # Anomaly sample: zero out KL to keep advantage at zero
            reverse_kls.append(torch.zeros_like(student_log_probs[i]))
            continue

        s_logp = student_log_probs[i]
        t_logp = teacher_log_probs[i]

        if opd_branches is not None and i < len(opd_branches):
            branch = opd_branches[i]
        else:
            branch = "reverse_kl"  # backward-compatible default

        if branch == "reverse_kl":
            # Reverse KL: penalize student for deviating from teacher on bad paths
            # advantage -= coef * (student_logp - teacher_logp)
            kl_penalty = reverse_kl_coef * (s_logp - t_logp)
            advantages[i] = adv - kl_penalty
            reverse_kls.append(s_logp - t_logp)

        elif branch == "forward_kl":
            # Forward KL: pull student toward teacher's efficient trajectory
            # advantage -= coef * (teacher_logp - student_logp)
            # Equivalently: advantage += coef * (student_logp - teacher_logp) with negated sign
            kl_penalty = forward_kl_coef * (t_logp - s_logp)
            advantages[i] = adv - kl_penalty
            # Store as student-teacher diff for unified logging
            reverse_kls.append(s_logp - t_logp)

        else:  # "skip"
            # No OPD signal: leave advantage unchanged
            reverse_kls.append(torch.zeros_like(s_logp))

    # Store reverse KL for logging (unified view regardless of branch)
...
def compute_advantages_and_returns(args: Namespace, rollout_data: RolloutBatch) -> None:
    ...
    # ============================================================
    # ★ 处理异常样本（raw_reward <= -99）
    #   1) loss_mask 置 0 → 切断 KL/entropy/policy 梯度
    #   2) 记录 invalid_indices，后面再清 advantage/returns 双保险
    #   注意：上游 post_process_rewards 已经把 rewards 设为 0 了，
    #         这里 rewards 数值不需要再动；raw_reward 保留供下游统计
    # ============================================================
    raw_rewards = rollout_data.get("raw_reward")
    invalid_indices: list[int] = []
    if raw_rewards is not None:
        for i, r in enumerate(raw_rewards):
    ...
    # ============================================================
    # ★ 双保险：清零异常样本的 advantage / returns
    #   防止后续 normalize_advantages (whitening) 或日志统计被极端值污染
    # ============================================================
